...
io.on('connection', (socket) => {
    ...
    {
    'result': 'fail',
    'message': the re ason for failure
    }
*/
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            ...
            socket.emit('invite_response', response);
            serverLog('invite room command failed', JSON.stringify(response));
            return;
        }
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
                result: 'fail',
                message: 'the user does not have a name registered'
            }
            socket.emit('invite_response', response);
            serverLog('invite room command failed', JSON.stringify(response));
            return;
        ...
        io.in(room).allSockets().then((sockets) => {
            ...
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.has(requested_user)) {
                ...
                socket.emit('invite_response', response);
                serverLog('invite_room command failed', JSON.stringify(response));
                return;
            }
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            ...
            socket.emit('game_start_response', response);
            serverLog('game_start room command failed', JSON.stringify(response));
            return;
        }
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
    ...
    'message': the mesage that was sent
    }
    or
    {
    'result': 'fail',
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        /* Make sure the current attempt is by the right color */
...
/* Return true if r+ dr supports playing at r + dc supprts playing */
