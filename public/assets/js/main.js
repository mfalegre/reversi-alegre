...
socket.on('join_room_response', (payload) => {
    ...
    /* If we are being notified of ourselves, that ignore message and return */
...
socket.on('game_update', (payload) => {
    ...
    if ((typeof board == 'undefined') || (board === null)) {
        console.log('Server did not load a valid board to display');
        return;
    ...
    if (payload.game.whose_turn === 'white') {
        $("#my_color").append('<h4>It is light pink\'s turn</h4>');
    }
    else if (payload.game.whose_turn === 'black') {
        $("#my_color").append('<h4>It is dark purple\'s turn</h4>');
    }
    else {
